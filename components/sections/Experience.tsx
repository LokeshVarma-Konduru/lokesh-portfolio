...
export function Experience() {
  const timelineData = experience.map((exp) => ({
    title: exp.company,
    content: (
      <div key={exp.company} className="rounded-xl border border-border bg-surface p-8">
        <div className="mb-4 inline-flex items-center rounded-lg border border-border bg-white p-2">
          <Image
            src={exp.logo}
            alt={exp.company}
            width={exp.logoWidth}
        ...
        <div className="flex flex-wrap items-center gap-2">
          <h4 className="text-xl font-semibold tracking-[-0.02em] text-foreground md:text-2xl">
            {exp.role}
          </h4>
          {exp.note && (
